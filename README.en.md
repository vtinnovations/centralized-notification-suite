...
**V-T.ONE Licence management**, the section every V-T.ONE package shares. It offers:

- **Verify and Activate Licence** — enter the key in the licence field and activate the
  installation.
- **Update Licence** — reconciles the stored state with the licence service. Leave the key
  field empty to re-check the stored key.
- **Remove Licence** — returns the installation to the unlicensed state immediately.

The section is laid out as one card per V-T.ONE package, matching the other packages in the
licence manager: the product name above the card, the current state on its first line, and
below it the key field and the three buttons.

Status, package, licensed host, covered hosts, version and term are displayed. The key itself
is never rendered back into the interface — the field is empty on every page load, including
directly after an activation.
...
centralized_notification_suite:
    log:
        ...
        retry_failed: true
    mailer:
        php_binary: ''          # empty means auto-detect the PHP CLI binary
        process_timeout: 120    # seconds before the cache rebuild is abandoned
        memory_limit: '-1'      # for the rebuild subprocess only; '' keeps the CLI php.ini value
```
...
| The file picker opens as a full page, with no Apply or Cancel | Another extension has loaded jQuery into the backend without `noConflict()`, taking the global `$` away from MooTools. This bundle ships a shim that restores it on its own screens; if the picker is also broken on core screens, fix the extension that loads jQuery. |
