...
**V-T.ONE Licence management**, dem gemeinsamen Abschnitt aller V-T.ONE-Pakete. Dort stehen zur
Verfügung:

- **Lizenz prüfen und aktivieren** – den Schlüssel im Lizenzfeld eintragen und die Installation
  aktivieren.
- **Lizenz aktualisieren** – gleicht den gespeicherten Stand mit dem Lizenzdienst ab. Bleibt das
  Schlüsselfeld leer, wird der gespeicherte Schlüssel erneut geprüft.
- **Lizenz entfernen** – setzt die Installation sofort in den nicht lizenzierten Zustand zurück.

Der Abschnitt ist als eine Karte je V-T.ONE-Paket aufgebaut, passend zu den übrigen Paketen in
der Lizenzverwaltung: der Produktname über der Karte, der aktuelle Zustand in deren erster
Zeile, darunter das Schlüsselfeld und die drei Schaltflächen.

Angezeigt werden Status, Paket, lizenzierter Host, abgedeckte Hosts, Version und Laufzeit. Der
Schlüssel selbst wird nie erneut in der Oberfläche ausgegeben – das Feld ist bei jedem
Seitenaufruf leer, auch unmittelbar nach einer Aktivierung.
...
centralized_notification_suite:
    log:
        ...
        retry_failed: true
    mailer:
        php_binary: ''          # leer bedeutet automatische Erkennung der PHP-CLI-Binärdatei
        process_timeout: 120    # Sekunden, bis der Cache-Neuaufbau abgebrochen wird
        memory_limit: '-1'      # nur für den Neuaufbau-Subprozess; '' übernimmt den Wert der CLI-php.ini
```
