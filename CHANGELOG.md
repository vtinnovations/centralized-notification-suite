...
- **Licence activation (V-T.ONE).** The suite now requires an activated licence before it
  dispatches anything. Activation lives in **Contao → Settings** under "Centralized
  Notification Suite Licence management": enter the key and save. The section also offers
  Update licence and Remove licence, and shows status, package, licensed host, covered hosts,
  version and term.
