...
class ExchangeJournal
{
    ...
    public function claim(string $requestId, string $rawBody, string $nonce, int $version, int $now): ExchangeOutcome
    {
        ...
        if (false === $existing) {
            // Two ways to get here, and both are refusals. Either the row disappeared between
            // the two statements -- pruned, or removed by hand -- or the insert was rejected
            // by the unique index on the nonce rather than the one on the request id, which
            // means a nonce that has already been spent is being presented under a fresh id.
            // That is a replay, and a replay must not be answered as a duplicate.
