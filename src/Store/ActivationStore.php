...
class ActivationStore
{
    ...
    private const WATERMARK = 'watermark.json';
     ...
     *
     * The watermark deliberately stays. It records the highest authoritative version this
     * installation has ever accepted, and clearing it here would turn the administrator's own
     * "Remove licence" button into the way to undo a revocation: remove, restore yesterday's
     * record.json from a backup, and the site is licensed again. Keeping it costs a legitimate
     * re-activation nothing, because the issuer never reissues at a lower version.
     */
    ...
    /**
     * The highest authoritative version ever accepted, and the state it arrived in.
     *
     * This is the floor every later record is measured against. Without it, a revocation is
     * undone by putting an older -- entirely genuine, correctly signed -- record.json back in
     * place, because every signature check passes on it. Signatures prove where a record came
     * from; only a monotonic version proves it is not yesterday's.
     *
     * @return array{version: int, status: string}|null
     */
    public function watermark(): array|null
    {
        $handle = $this->lock(LOCK_SH);

        try {
            $raw = @file_get_contents($this->path(self::WATERMARK));
            ...
            if (!\is_array($decoded) || !\is_int($decoded['version'] ?? null) || !\is_string($decoded['status'] ?? null)) {
                return null;
            }

            return ['version' => $decoded['version'], 'status' => $decoded['status']];
        } finally {
            $this->release($handle);
        }
    }
    ...
    /**
     * Raises the watermark to a newly accepted version.
     *
     * Never lowered: a caller passing an older version is recording something that has already
     * been superseded, and honouring it would reopen the downgrade this file exists to close.
     * The status of the highest version is kept with it, so a later record at that same version
     * cannot flip a withdrawal back into an entitlement.
     *
     * @throws StateNotWritable
     */
    public function raiseWatermark(int $version, string $status): void
    {
        $this->prepareDirectory();
        $handle = $this->lock(LOCK_EX);
        ...
        try {
            ...
            if (\is_array($current) && \is_int($current['version'] ?? null) && $current['version'] > $version) {
                return;
            }

            $encoded = json_encode(['version' => $version, 'status' => $status], JSON_UNESCAPED_SLASHES);

            if (false === $encoded) {
                throw new StateNotWritable('The watermark cannot be serialised.');
            }

            $tmp = $this->stage(self::WATERMARK, $encoded);

            if (!@rename($tmp, $this->path(self::WATERMARK))) {
                @unlink($tmp);

                throw new StateNotWritable('The watermark could not be activated.');
            }
        } finally {
            $this->release($handle);
        }
