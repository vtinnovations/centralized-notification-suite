...
class StateUpdateEndpoint
{
    ...
    private function apply(\stdClass $body, string $raw, int $now): Response
    {
        ...
        try {
            ...
            // thing standing between a revocation and its undo is this comparison.
            if ($record->version < $this->gate->current()->version()) {
        ...
        $this->logger->info(\sprintf('An inbound state update applied version %d.', $record->version));
